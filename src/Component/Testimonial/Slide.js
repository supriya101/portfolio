...
const Slide = ({ id, image, design, name, post, date, desc, skills, desc2, valueIndex, index }) => {
  let position = "nextSlide"
  if (valueIndex === index) {
    position = "activeSlide"
  }
  if (valueIndex === index - 1 || (index === 0 && valueIndex === Testimonial_Api.length - 1)) {
    position = "lastSlide"
  }
      ...
      <article className={`d_flex ${position}`} key={id}>
        ...
        <div className='left box_shodow'>
          ...
          <div className='details mtop'>
            ...
            <h2 className='exp-title'>{name}</h2>
          </div>
        </div>
